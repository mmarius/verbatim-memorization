...
# TODO(mm): implement ACR memorization based on GCG


def get_memorized_sequences(
    model,
    tokenizer,
    sequences,
    prompt_lengths=None,
    ...
    # create a dictionary mapping each sequence to a dictionary of prefixes and ground-truth completions (labels)
    ...
    prompts = list([p for pl in sequence_to_prompt_and_label.values() for p in pl])
    ...
    # compute the longest common prefix between the prediction and the ground-truth completion
    ...
    for sequence in sequence_to_prompt_and_label:
        for p, label in sequence_to_prompt_and_label[sequence].items():
            out = prompt_to_output[p]  # the completion predicted by the model

            # TODO(mm): figure out what this can be used for
            # if filter_fn is not None and not filter_fn(p + out):
            #     continue

            # compute the longest common prefix between the predicted completion and the actual completion
            common = os.path.commonprefix(
                [ascii_letter_normalize(out), ascii_letter_normalize(label)]
            )
            mem_len = len(tokenizer(common).input_ids)

            # exclude completion that are part of the prefix
            if mem_len > 5 and (common in p or out[:32] in p):
                # if debug:
                #     print("COPYING")
                sequence_to_memorized[sequence][p] = "COPYING"
                continue
            
            # save the longest common prefix for a given prompt as the memorized completion
            # since we are looking for prefixes, we can simply take the first len(common) tokens
